...
WGE.Piano = WGE.Class(WGE.SlideshowInterface,
{
	config : 1,
	audioFileName : ["slideshow_photoframe.mp3", "slideshow_photoframe.ogg"],  // 音乐文件名
	...
	_initTimeline : function()
	{
		...
		var timeStamp = 0;
		var z = 0;
		var imgs = WGE.clone(this.srcImages);

		for(var j = 0; j < imgs.length; j += 20)
		{
			for(var i = 0; i != 4; ++i)
			{
				var sceneArr = FTPhotoFrame.initScene(imgs, this.canvas.width, this.canvas.height, z, timeStamp, 17000);
				z += 10000;
				timeStamp += 15000;
				this.timeline.pushArr(sceneArr);
			}
		}
	},
